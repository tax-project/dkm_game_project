...
public interface GiftRankingDao extends IService<GiftRankingEntity> {
    ...
    @Select("SELECT gr.*,u.we_chat_nick_name,u.we_chat_head_img_url FROM (SELECT * FROM tb_gift_ranking where send>0 ORDER BY send desc LIMIT 20) gr LEFT JOIN tb_user u on u.user_id = gr.user_id")
    List<GiftRankingDto> getGiftRankingSend();
    @Select("SELECT gr.*,u.we_chat_nick_name,u.we_chat_head_img_url FROM (SELECT * FROM tb_gift_ranking where accept>0 ORDER BY accept desc LIMIT 20) gr LEFT JOIN tb_user u on u.user_id = gr.user_id")
