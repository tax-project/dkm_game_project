...
public class TbEquipmentControllerTwo {
    @Autowired
    ITbEquipmentServiceTwo tbEquipmentService;
   /* @Autowired
    ITbKnapsackServiceTwo tbKnapsackService;*/
    @Autowired
    ...
    public List<TbEquipmentVo> selectByKnapsackId(){
        ...
        TbKnapsackTwo tbKnapsack=new TbKnapsackTwo();
        tbKnapsack.setUserId(user.getId());
        //根据登录的用户id查询主键出背包主键
        List<TbKnapsackTwo> listOne=tbKnapsackService.findById(tbKnapsack);
        List<TbEquipmentVo> list=null;
        TbEquipmentVo tbEquipment=new TbEquipmentVo();
        //判断用户背包添加过
        if(!StringUtils.isEmpty(listOne)){
            for (TbKnapsackTwo knapsack : listOne) {
